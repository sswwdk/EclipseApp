...
**응답 예시 (태그 추출 및 Next/More 버튼):**
```json
{
  "status": "success",
  "message": "현재까지 수집된 키워드: 조용한, 와이파이, 공부, 집중, 인터넷",
  "stage": "collecting_details",
  "tags": ["조용한", "와이파이", "공부", "집중", "인터넷"],
...
```json
{
  ...
  "availableCategories": ["카페", "음식점"]
}
```

**응답 예시 (Yes 클릭 후 - 추천 완료):**
```json
{
  "status": "success",
...
1. **haru_gpt_api.py** (새로 생성 및 지속 개선)
   - FastAPI 서버 구현
   - LangChain 기반 대화 처리
   - 세션 관리 기능 (waitingForUserAction 상태 관리)
   - 카테고리별 맞춤 프롬프트 적용
   - 하드코딩된 추천 데이터베이스 구현
...
2. **lib/services/openai_service.dart** (수정)
   ...
   - HTTP 통신 구현
   - 구조화된 응답 처리 (showYesNoButtons, yesNoQuestion 등)
