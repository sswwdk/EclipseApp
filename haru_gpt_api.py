...
system_prompt = """
당신은 사용자의 답변을 의미 있는 키워드로 태그화하는 전문가입니다.

**태그화 규칙:**
1. 사용자의 문장에서 핵심 키워드만 추출합니다.
2. 감정적이거나 추상적인 단어('좋은', '멋진', '재미있는' 등)는 제외합니다.
3. 구체적인 상황, 공간, 행동, 특징 중심으로 태그화합니다.
4. 카테고리별로 관련된 키워드만 추출합니다.

**출력 형식:**
- 키워드만 쉼표로 구분하여 나열
- 다른 설명이나 문장 없이 키워드만 출력
- 5~6개의 키워드 추출
    ...
    try:
        ...
        category_prompts = {
            ...
            **제외할 키워드:**
            - 감정적 표현 ('좋은', '멋진', '재미있는' 등)
            - 일반적 표현 ('편한', '괜찮은' 등)
            - 인원 수를 2명 이상 선택했는데 키워드로 '혼자'라는 키워드는 제공하지 않는다
...
def handle_user_message(session: Dict, user_message: str) -> ChatResponse:
    ...
    # 카테고리별 태그 추출 (LLM 사용)
    new_tags = extract_tags_by_category(user_message, current_category)
...
def handle_user_action_response(session: Dict, user_response: str) -> ChatResponse:
    ...
    """
    # 응답 파싱
    is_next = any(word in user_response.lower() for word in ["yes", "네", "예", "좋아", "그래", "맞아", "ㅇㅇ"])
    is_more = any(word in user_response.lower() for word in ["추가", "더", "더해", "추가하기"])
...
async def confirm_results(request: ChatRequest):
    ...
    try:
        ...
        # Yes/No 응답 파싱
        is_yes = any(word in request.message.lower() for word in ["yes", "네", "예", "좋아", "그래", "맞아", "ㅇㅇ"])
