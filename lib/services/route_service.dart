...
class RouteResult {
  ...
  factory RouteResult.fromJson(Map<String, dynamic> json) {
    ...
    if (json.containsKey('duration_seconds')) {
      ...
      if (duration is int) {
        durationSeconds = duration; // 분 단위 값을 durationSeconds에 저장
    ...
    if (durationSeconds != null) {
      if (isAlreadyInMinutes) {
        ...
        print('   durationMinutes (이미 분 단위): $durationMinutes');
      } else {
        durationMinutes = (durationSeconds / 60).round();
        print('   durationSeconds: $durationSeconds -> durationMinutes: $durationMinutes');
      }
    }
    ...
    if (routes != null && routes.isNotEmpty) {
      ...
      if (stepsData != null && stepsData.isNotEmpty) {
        steps = stepsData.map((s) => RouteStep.fromJson(s as Map<String, dynamic>)).toList();
      }
    }
...
class RouteStep {
  ...
  factory RouteStep.fromPublicTransportRoute(Map<String, dynamic> json) {
    ...
    if (description.contains('호선') || 
      ...
      type = 'transit';
    } else if (description.contains('도보') || 
               description.contains('걸어서')) {
      type = 'walk';
    } else if (description.contains('자동차') || 
               description.contains('차로')) {
      type = 'drive';
    }
  ...
  static Future<RouteResult> calculateRoute({
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({
          'origin': [origin.lng, origin.lat], // tuple 형식: [위도, 경도]
          'destination': [destination.lng, destination.lat], // tuple 형식: [위도, 경도]
          'transport_type': transportType.toString(), // 문자열로 변환: "0", "1", "2"
        }),
      ).timeout(
      ...
      if (response.statusCode == 200) {
        ...
        print('   responseData 키: ${responseData is Map ? (responseData as Map).keys.toList() : 'N/A'}');
        
        final data = responseData['data'] ?? responseData;
        print('   파싱할 data: $data');
        print('   data 키: ${data is Map ? (data as Map).keys.toList() : 'N/A'}');
